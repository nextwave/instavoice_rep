...
- (void)replyToHTTPRequest
{
	...
	DLog(@"HTTPConnection:%p replyToHTTPRequest:\n%@", self, tempStr);
...
- (NSString *)filePathForURI:(NSString *)path
{
	...
	{
		DLog(@"no document root configured");
		return nil;
	}
	...
	if([relativePath hasSuffix:@"/"])
	{
        ...
        path1 = [NSString stringWithFormat:@"%@%@",[[server documentRoot] path],completedRelativePath];
	}
	else
	{
        //WUMING: 下载的文件位置位于  Recordings 目录。
        //ServerRoot 目录位于  Web 目录。
        if ([relativePath hasPrefix:@"Recordings"]) {
            NSString* documentRoot = [[server documentRoot] path];
            NSRange rang = [documentRoot rangeOfString:@"/" options:NSBackwardsSearch];
            documentRoot = [documentRoot substringToIndex:rang.location];
            
            path1 = [NSString stringWithFormat:@"%@/%@",documentRoot,relativePath];
        }
        else
        {
            path1 = [NSString stringWithFormat:@"%@/%@",[[server documentRoot] path],relativePath];
        }
	}
...
//		filePath = [filePath substringFromIndex:1];
	...
	{
		DLog(@"file exist");
		return [[[HTTPFileResponse alloc] initWithFilePath:filePath fileName:path] autorelease];
...
- (void)handleVersionNotSupported:(NSString *)version
{
	...
	DLog(@"HTTP Server: Error 505 - Version Not Supported: %@", version);
...
- (void)handleAuthenticationFailed
{
	...
	DLog(@"HTTP Server: Error 401 - Unauthorized");
...
- (void)handleInvalidRequest:(NSData *)data
{
	...
	DLog(@"HTTP Server: Error 400 - Bad Request");
...
- (void)handleUnknownMethod:(NSString *)method
{
	...
	DLog(@"HTTP Server: Error 405 - Method Not Allowed: %@", method);
...
- (void)handleResourceNotFound
{
	...
	DLog(@"HTTP Server: Error 404 - Not Found");
...
{
//	DLog(@"HTTPConnection: onSocket:willDisconnectWithError: %@", err);
}
