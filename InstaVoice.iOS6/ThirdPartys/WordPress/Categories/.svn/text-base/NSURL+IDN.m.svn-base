...
+ (NSString *)_punycodeEncode:(NSString *)aString;
{
    ...
    while (handled < stringLength) {
        ...
        for (inputPtr = inputBuffer; inputPtr < inputEnd; inputPtr++) {
            if (*inputPtr < n) 
                ...
                // DLog(@"encode: delta=%d pos=%d bias=%d codepoint=%05x", delta, inputPtr-inputBuffer, bias, *inputPtr);
    ...
    *outputPtr = '\0';
#ifdef DEBUG_toon    
    DLog(@"Punycode encoded \"%@\" into \"%s\"", aString, outputBuffer);
#endif    
    ...
    /* Convert the variable-length-integers in the deltas section into machine representation */
    {
        ...
        for(i = 0; i < deltas.length; i++) {
            ...
            if (digit < threshold) {
                ...
                // DLog(@"decode: delta[%d]=%d bias=%d from=%@", deltaCount-1, value, bias, [aString substringWithRange:(NSRange){deltas.location + i - position, position+1}]);
